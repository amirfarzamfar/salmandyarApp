...
import { AssessmentAssignment, QuestionAnswerDetail } from '@/types/assessment-assignment';
...
export function AssessmentResultView({ assignment, onClose }: AssessmentResultViewProps) {
    const [detail, setDetail] = useState<AssessmentAssignment | null>(null);
    const [loading, setLoading] = useState(true);

    useEffect(() => {
        const fetchDetail = async () => {
            try {
                const data = await assessmentAssignmentService.getAssignmentById(assignment.id);
                setDetail(data);
            } catch (error) {
                toast.error('خطا در دریافت جزئیات آزمون');
            } finally {
                setLoading(false);
            }
        };
        ...
        return (
            <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                <div className="bg-slate-900 border border-slate-700 rounded-xl p-6 text-center">
                    ...
                    <p className="text-slate-400 mt-2">اطلاعات پاسخنامه برای این آزمون موجود نیست.</p>
    ...
    // Assuming each question has at least one tag or "General"
                ...
                <div className="flex items-center justify-between p-6 border-b border-slate-800 bg-slate-800/50">
                    <div>
                        <h2 className="text-xl font-bold text-white flex items-center gap-2">
                            ...
                            نتایج آزمون: {assignment.formTitle}
                        </h2>
                        <div className="mt-1 text-sm text-slate-400">
                            کاربر: <span className="text-white">{assignment.userFullName}</span> | 
                            تاریخ آزمون: <span className="text-white">{new Date(submission?.submittedAt || '').toLocaleString('fa-IR')}</span>
                        </div>
                    </div>
